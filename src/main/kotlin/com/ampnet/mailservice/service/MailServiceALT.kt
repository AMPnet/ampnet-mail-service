...
abstract class MailServiceALT(
    ...
    abstract fun sendMail(mailTextInput: MailText)

    fun fillTemplate(template: Mustache, data: Any): String {
        val writer = StringWriter()
        template.execute(writer, data).flush()
        return writer.toString()
    }

    fun createMailMessage(to: List<String>, subject: String, text: String): List<MimeMessage> {
    ...
    fun sendEmails(mails: List<MimeMessage>, notifySenderOnError: (List<MimeMessage>) -> Unit = {}) {
    ...
    fun sendEmail(mail: MimeMessage): Boolean {
        return try {
